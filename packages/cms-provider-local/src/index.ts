...
export const localCmsStorageCapabilities = Object.freeze({
  schedule: true,
  media: false,
  webhook: false,
  release: false,
...
export const localCmsCapabilities: CmsProviderCapabilities = Object.freeze({
  supported: [
    "content.readDraft",
    "content.write",
    "content.publish",
...
export type LocalCmsDatabase = Client;

export function createLocalCmsDatabase(input?: { readonly url?: string }) {
  return createClient({ url: input?.url ?? "file:.agency-cms/content.db" });
}

const localSchema = `
CREATE TABLE IF NOT EXISTS agency_cms_local_state (
  namespace TEXT PRIMARY KEY NOT NULL,
  schema_version INTEGER NOT NULL,
  payload TEXT NOT NULL,
  updated_at TEXT NOT NULL
);
INSERT OR IGNORE INTO agency_cms_local_state (
  namespace, schema_version, payload, updated_at
) VALUES ('default', 1, '{"documents":[],"revisions":[]}', CURRENT_TIMESTAMP);
`;

export async function applyLocalCmsMigrations(database: LocalCmsDatabase) {
  await database.executeMultiple(localSchema);
}
...
type LocalCmsAuthorizationContext = Readonly<{
  action: CmsCollectionAction;
  collection: CmsCollectionDefinition;
  actorId: string | undefined;
}>;

export type LocalCmsCollectionProviderOptions = Readonly<{
  database: LocalCmsDatabase;
  registry: CmsCollectionRegistry;
  namespace?: string;
  createId?: () => string;
  now?: () => Date;
  authorize?: (context: LocalCmsAuthorizationContext) => void | Promise<void>;
}>;
...
function parseStoredState(value: unknown): StoredState {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return cmsError("MIGRATION_FAILED", "Local CMS state is not an object.");
  }
  const state = value as Partial<StoredState>;
  if (!Array.isArray(state.documents) || !Array.isArray(state.revisions)) {
    return cmsError("MIGRATION_FAILED", "Local CMS state is incomplete.");
  }
  return {
...
function decodeState(payload: unknown) {
  if (typeof payload !== "string") {
    return cmsError("MIGRATION_FAILED", "Local CMS state payload is missing.");
  }
  try {
    return parseStoredState(JSON.parse(payload));
  } catch (error) {
    if (error instanceof CmsError) throw error;
    return cmsError("MIGRATION_FAILED", "Local CMS state is not valid JSON.");
  }
}
...
function safeNamespace(value: string) {
  if (!/^[a-z][a-z0-9_-]{0,63}$/.test(value)) {
    return cmsError(
      "VALIDATION_FAILED",
      "Local CMS namespace must be a safe lowercase identifier.",
    );
  }
  return value;
}
...
export class LocalCmsCollectionProvider implements CmsCollectionPortabilityProvider {
  readonly registry: CmsCollectionRegistry;
  readonly capabilities = localCmsCapabilities;
  readonly #database: LocalCmsDatabase;
  readonly #namespace: string;
  readonly #createId: () => string;
  readonly #now: () => Date;
  readonly #authorize?: LocalCmsCollectionProviderOptions["authorize"];

  constructor(options: LocalCmsCollectionProviderOptions) {
  ...
  async #load(executor: Pick<LocalCmsDatabase, "execute">) {
    const result = await executor.execute({
      sql: "SELECT payload FROM agency_cms_local_state WHERE namespace = ? LIMIT 1",
      args: [this.#namespace],
    });
    if (!result.rows.length) {
      await executor.execute({
        sql: `INSERT INTO agency_cms_local_state (
          namespace, schema_version, payload, updated_at
        ) VALUES (?, 1, ?, ?)`,
        args: [
          this.#namespace,
          JSON.stringify({ documents: [], revisions: [] }),
          this.#now().toISOString(),
        ],
      });
      return { documents: [], revisions: [] } satisfies StoredState;
    }
  ...
  async #mutate<T>(operation: (state: StoredState) => T | Promise<T>) {
    ...
    try {
      const state = await this.#load(transaction);
      const result = await operation(state);
      await transaction.execute({
        sql: `UPDATE agency_cms_local_state
          SET schema_version = 1, payload = ?, updated_at = ?
          WHERE namespace = ?`,
        args: [
...
export function createLocalCmsCollectionProvider(
  options: LocalCmsCollectionProviderOptions,
) {
  return new LocalCmsCollectionProvider(options);
}
