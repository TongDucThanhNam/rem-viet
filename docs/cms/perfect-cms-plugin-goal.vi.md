...
| Visual editor                     | PARTIAL  | một protocol và UX cho mọi document type, inline edit, outline, patterns, copy/paste          | secure preview v2, patterns và inline-text contract đã proven; còn outline/copy-paste, UX chung đầy đủ và final-candidate browser matrix |
...
Increment mới nhất là permissioned inline text tại `8fc914c`, tiếp nối visual
component patterns/presets tại `c67f225`. Kernel, Rèm live standard-page canvas
và Atelier reuse proof đều dùng cùng opt-in field contract; exact-commit packed
consumer đã pass. Các increment này tiếp nối complete Admin Platform v2
localization tại `982f3a4` và reusable content tại `baf899a`; các checkbox final
