...
- Real notification delivery, real-user p75 vitals, a non-developer pilot and
  clean-checkout release provenance remain external gates. The isolated remote
  restore and isolated Acme staging runtime are now proven and cleaned up.
...
| Backup and restore rehearsed                                             | PARTIAL  | `site:backup` exported real staging D1 and passed unique local plus isolated remote restores. The remote drill restored the exact archived SHA-256 into a separate APAC D1, proved `quick_check=ok`, 26-table and exact critical-row parity in 2.11 minutes, then reverified parity and deleted the target. A separate `rem-viet-backups` R2 bucket has managed `r2.dev` disabled, zero enabled custom domains and a 365-day `d1/` lock; both direct and composed periodic uploads were downloaded and hash/size verified. `site:backup:github:audit` now verifies the exact default-branch workflow, redacted configuration, run-bound restore/archive artifacts, retention and manual→weekly sequence. Its live audit correctly reports the workflow absent from remote `main`, all four settings absent and no qualifying runs. Production pre-migration timing also remains external.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| Client-ready evidence and tag gate    | PARTIAL  | Schema v3, sanitized live `release:readiness`, live Worker provenance, commit/clean-checkout verifier and tag-triggered CI contract fail closed on dirty/mismatched deployment, missing pilot, p75, exactly-once Resend, enabled/delivered Cloudflare alert, isolated staging restore, second-site, manual/weekly immutable backup receipts or production backup proof. The new read-only `release:github:audit` additionally hashes the exact default-branch workflow and requires an active GitHub Actions registration. Its live run reports both absent from remote `main`, so the gate is implemented locally but not activated and cannot be called proven for release. |
...
2. Repeat the now-passing Acme deployment/smoke from the exact clean release
   ...
   Playwright projects; the current live Worker intentionally reports dirty
