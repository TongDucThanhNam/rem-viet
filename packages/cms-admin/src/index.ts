...
/** Starts a bounded local command history around one draft value. */
...
/**
 * Commits a draft value while coalescing rapid edits in the same semantic group.
 * Any new commit invalidates the redo branch.
 */
...
/** Moves one step back without mutating any recorded draft value. */
...
/** Moves one step forward after an undo. */
...
export function createCmsVisualEditorSelectionMessage(
  ...
  if (!blockId.trim()) throw new Error("Visual editor block ID is required.");
  if (fieldPath !== undefined && !isValidVisualFieldPath(fieldPath)) {
...
export function createCmsVisualEditorMoveMessage(
  ...
  if (!isValidVisualBlockId(blockId) || !isValidVisualBlockId(targetBlockId)) {
...
export function createCmsVisualEditorInsertMessage(
  ...
  if (!isValidVisualBlockType(blockType)) {
    throw new Error("Visual editor insert requires a valid block type.");
  }
  if (!isValidVisualBlockId(targetBlockId)) {
...
export function createCmsVisualEditorDuplicateMessage(
  ...
  if (!isValidVisualBlockId(blockId)) {
...
function isValidVisualFieldPath(value: string) {
...
function isValidVisualBlockId(value: unknown): value is string {
  return (
    typeof value === "string" &&
    value.trim().length > 0 &&
    value.length <= 128 &&
...
function isValidVisualBlockType(value: unknown): value is string {
...
export function isCmsVisualEditorMessage(
  ...
  if (candidate.type === "select") {
    return (
      isValidVisualBlockId(candidate.blockId) &&
      (candidate.fieldPath === undefined ||
        (typeof candidate.fieldPath === "string" &&
          isValidVisualFieldPath(candidate.fieldPath)))
    );
  }
  if (candidate.type === "move") {
    return (
      isValidVisualBlockId(candidate.blockId) &&
      isValidVisualBlockId(candidate.targetBlockId) &&
  ...
  if (candidate.type === "insert") {
    return (
      isValidVisualBlockType(candidate.blockType) &&
      isValidVisualBlockId(candidate.targetBlockId) &&
      (candidate.placement === "before" || candidate.placement === "after")
    );
  }
  if (candidate.type === "duplicate" || candidate.type === "remove") {
    return isValidVisualBlockId(candidate.blockId);
  }
  return (
    candidate.type === "state" &&
    Array.isArray(candidate.blocks) &&
    (candidate.selectedBlockId === null ||
      isValidVisualBlockId(candidate.selectedBlockId)) &&
    (candidate.selectedFieldPath === null ||
      (typeof candidate.selectedFieldPath === "string" &&
        isValidVisualFieldPath(candidate.selectedFieldPath))) &&
