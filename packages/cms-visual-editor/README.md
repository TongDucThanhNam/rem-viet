...
targets, a permission-aware nested outline and framework-neutral tree keyboard
reducer, bounded composition, permissions, commands, local history, document
...
trusted as authority. The outline derives parent/slot/depth/selection and action
availability only from the validated canonical document and current grants;
labels remain template-owned. Inline editing is opt-in per text field; discovery
and mutation both enforce component and field grants, and accepted text is
