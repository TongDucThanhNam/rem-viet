...
## Payload-grade competitiveness matrix — 2026-08-20

This matrix is deliberately narrower than a claim of complete Payload feature
parity. `PROVEN` means the repository has executable local evidence for the
agency use case. `PARTIAL` means a useful bounded capability exists but an
important product or operational slice remains. `OPEN` means the capability is
not implemented or has no valid evidence.

| Capability                        | Status      | Repository evidence and honest boundary                                                                                                                                                                                                                                                                                                                                                                                  |
| --------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Schema-generated admin            | **PROVEN**  | Collection definitions generate accessible list, filter, create and edit surfaces; field registries allow template-owned controls. Visual block definitions generate inspector fields through the template factory.                                                                                                                                                                                                      |
| REST/server APIs                  | **PROVEN**  | Typed server SDK and allow-listed REST resources enforce capabilities, bounds, parsing and sanitized errors. Automatic GraphQL is intentionally not implemented.                                                                                                                                                                                                                                                         |
| Authentication and access control | **PROVEN**  | Better Auth sessions, staff roles, server capabilities, provider authorization, component permissions and field permissions fail closed in tests.                                                                                                                                                                                                                                                                        |
| Drafts, versions and autosave     | **PROVEN**  | Draft isolation, autosave/flush, optimistic conflicts, immutable publication, revisions and restore have provider, unit and authenticated two-tab browser coverage.                                                                                                                                                                                                                                                      |
| Scheduling and jobs               | **PARTIAL** | Schedule/unschedule and scheduled publication contracts are proven. A general durable jobs/queue product comparable to a broad background-job system is not part of the kernel.                                                                                                                                                                                                                                          |
| Media/DAM                         | **PARTIAL** | R2 upload, metadata, reviewed alt text, usage lookup, reference-safe deletion and provider conformance are proven. Advanced DAM taxonomy, focal editing and transformations are provider/template concerns rather than one complete neutral product.                                                                                                                                                                     |
| Localization                      | **PROVEN**  | Shared/localized fields, locale-specific lifecycle, fallback metadata, relationships and generated locale controls pass generic provider and packed-consumer tests.                                                                                                                                                                                                                                                      |
| Relationships                     | **PROVEN**  | Typed to-one/to-many relationships, integrity checks, restrict/nullify policy, generated controls and atomic provider enforcement are covered.                                                                                                                                                                                                                                                                           |
| Hooks and plugins                 | **PROVEN**  | Instance-scoped typed modules and ordered lifecycle hooks are isolated and transaction-safe. Arbitrary editor-installed runtime code and a public plugin marketplace are explicit non-goals.                                                                                                                                                                                                                             |
| Visual authoring                  | **PARTIAL** | The neutral kernel provides canonical nodes, constraints, permissions, commands/history, migrations and adapters. Rèm preserves its production canvas; Atelier proves nine independent blocks and nested slots. The authenticated replay-safe v2 channel is adopted by the homepage pair, while standard-page/post routes remain on compatibility protocols and Atelier lacks a full authenticated browser editor shell. |
| Migrations                        | **PROVEN**  | Contiguous schema/content migrations, fail-closed future versions, empty/upgraded fixtures, backup-before-apply plans, N-to-N+1 migration and receipt-bound rollback all execute locally.                                                                                                                                                                                                                                |
| Observability                     | **PARTIAL** | Structured audit, health, performance and incident sources exist. Representative RUM, live alert dispatch and production operational receipts remain external/open evidence.                                                                                                                                                                                                                                             |
| Backups and restore               | **PARTIAL** | Backup archives, isolated restore verification and upgrade rollback are automated and tested; a current production pre-migration backup/restore receipt is necessarily external.                                                                                                                                                                                                                                         |
| Multitenancy                      | **PARTIAL** | Site/document identity is enforced in manifests, visual documents, preview messages and isolated per-site provider resources. The supported agency model is one isolated stack per client, not a shared-database multi-tenant SaaS control plane.                                                                                                                                                                        |

The current result is Payload-grade for the agency's bounded, code-owned
collection and lifecycle workflow—not a statement that every Payload feature,
adapter, deployment model or ecosystem integration has been reproduced.
