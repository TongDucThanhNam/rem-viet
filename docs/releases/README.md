...
Prepare the coordinated eleven-package bundle from an exact clean commit with:
...
exact eleven-package plan, release-input hashes, and registry non-existence. It
then rebuilds all eleven tarballs from the clean source and requires matching
...
it is not part of the stable eleven-package publication merely because local
