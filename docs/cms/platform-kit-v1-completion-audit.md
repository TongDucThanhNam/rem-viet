...
| Global settings/navigation cross neutral versioned provider ports            | Proven locally                 | The identical draft/publication/compensation conformance passes Cloudflare D1 and the structural Sanity document adapter; boundary tests, typed app adapter, real D1 release tests, and authenticated Worker draft-isolation/history/restore E2E also pass                                                                                                                            | Run the release/recovery flow on independent staging and retain a hosted Sanity global-content receipt |
...
| Version-bound editorial review crosses the neutral provider boundary                | Proven locally                 | Core request/decision schemas and capabilities, runtime immutable-event derivation/workflow/conformance, admin presentation/action state, and Cloudflare migration/provider are app-independent. The admin resolver independently requires request and decision grants, so write or publish authority cannot leak into review presentation. D1 conformance proves idempotent request, current-only queue, stale rejection, re-request, required change notes, approval and publication of only the exact approved version; Rèm Việt consumes the neutral derivation/presentation APIs and exact server-issued claims. A real D1-backed localized collection test proves collection/document/locale review identity, queue reconstruction, content-free notification, checklist and independent-reviewer gates, failed release preflight before approval, and successful publication afterward. API and focused Worker E2E prove role boundaries and arbitrary collection policy administration. The packed clean consumer executes the neutral lifecycle through public exports.                                                                                           |
| Versioned global settings and navigation recovery                                   | Proven locally                 | Generic keyed runtime ports pass identical D1 and structural Sanity conformance; both retain immutable revisions, isolate working drafts, publish explicitly, compensate exactly, and restore by appending a new draft version. The human-field Cloudflare admin uses expected versions and confirmation-gated restore; authenticated Worker E2E proves settings/navigation draft isolation while release tests prove publication, exact replay, content-free outbox delivery, and rollback to the prior public revision.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
## Hosted provider conformance update — 2026-08-21

Draft PR [#1](https://github.com/TongDucThanhNam/rem-viet/pull/1) made the CMS
conformance workflow remotely executable without merging it to `main`.
[GitHub Actions run 32494308357](https://github.com/TongDucThanhNam/rem-viet/actions/runs/32494308357),
bound to commit `c4b7c2f`, passed all three jobs: real PostgreSQL 17 plus pinned
MinIO provider tests/typecheck on Ubuntu, and the packed-consumer plus complete
clean-checkout/upgrade rehearsal on both Ubuntu and Windows. The initial hosted
attempt exposed a Linux-only peer-resolution dependency in the public-template
bundle proof. Later reruns exposed a preselected-port startup race and ANSI-coded
...
1. Exact clean release-source commit `35ff024162366e9616d26d89c836acbf9007cab1`
   ...
   Rèm Việt and an independent repo.
